...
export const Payment = () => {
    ...
    const [udata, setUdata]=useState("")
     ...
     useEffect(()=>{
        axios.get("http://localhost:8080/useraddress").then((res)=>setUdata(res.data[res.data.length-1]))
     },[])
  console.log(udata)
              ...
              <div className='payment-add-div'>
                ...
                <h3>SHIPPING</h3>
                   <h4>SHIPPING ADDRESS</h4>
                     <p>{udata.first_name} {udata.last_name}</p>
                     <p>{udata.address}</p>
                     <p>{udata.city} {udata.state} {udata.zipcode}</p>
                     <p>{udata.mobile}</p>
                </div>
                ...
                <h3 className='payment-cal-h3-div'>PAYMENT</h3>
                  <h4>ORDER DETAILS</h4>
                     <p>Prices are inclusive of all taxes</p>
                      ...
                      <button className="payment-cal-but" onClick={displayRazorpay}>PROCEED WITH PAYMENT</button>
              </div>
        </div>
    </>
  )
